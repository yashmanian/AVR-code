...
// now in timer definition: #define TIMER_PRESCALE_MASK		0x07	// Timer Prescaler Bit-Mask
...
static __inline__ void timerOff(const Timer* timer){
	_SFR_MEM8(pgm_read_word(&timer->pgm_prescaler.port)) &= ~ pgm_read_byte(&timer->pgm_prescaler.mask);
}
...
static __inline__ boolean timerIsInUse(const Timer* timer){
	return ((_SFR_MEM8(pgm_read_word(&timer->pgm_prescaler.port)) & pgm_read_byte(&timer->pgm_prescaler.mask)) != 0) ? TRUE : FALSE;
}
...
// or 0 if none
uint16_t timerNextPrescaler(uint16_t prescale, const Timer* timer);
