...
typedef struct s_marquee{
	const SEGLED_LIST* const leds;// An array of LEDs to scroll the msg across
...
// Convert a char to an 8 segment led display mask
...
public:
	...
	void on(SEGLED_SEGMENT seg) const{
		segled_on(m_cstr,seg);
	}
	void off(SEGLED_SEGMENT seg) const{
		segled_off(m_cstr,seg);
	}
	void set(SEGLED_SEGMENT seg, boolean val) const{
		segled_set(m_cstr,seg,val);
	}
	uint8_t put(uint8_t c) const{
		return segled_put_char(m_cstr,c);
	}
private:
	SEGLED* m_cstr;
};
...
public:
	...
	boolean isActive(void)const{
		return marqueeIsActive(m_cstr);
	}

	void setCharDelay(TICK_COUNT delay) const{
		marqueeSetCharDelay(m_cstr,delay);
	}

	void setEndDelay(TICK_COUNT delay) const{
		marqueeSetEndDelay(m_cstr,delay);
	}
