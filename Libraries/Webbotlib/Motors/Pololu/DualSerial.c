...
// Define the class
static const __ACTUATOR_DRIVER_CLASS PROGMEM c_Pololu_DualSerial = MAKE_ACTUATOR_DRIVER_CLASS(&setSpeed,&setConnected);
...
void pololuDualSerialInit(POLOLU_DS_DRIVER* driver){
	...
	for(int i=0;i<driver->num_devices;i++){
		...
		act->actuator.sclass = &c_Pololu_DualSerial;
		act->driver = driver;
		act_setSpeed(act,0);
