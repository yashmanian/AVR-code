...
#define MAKE_REMOTE_SERVO(inverted, center, range)  { MAKE_ACTUATOR(inverted),null,null,center, range, 0,0,0,0, null }
...
typedef struct s_servo_driver{
	const SERVO_LIST* const servos;		// The servos
...
#define servosCenter(servos,uart) _servosCenter(servos, (uint8_t)(sizeof(servos)/sizeof(SERVO*)))
...
public:
	...
	void setConfig(uint16_t center, uint16_t range){
		servoSetConfig(m_cstr,center, range);
	}
...
public:
	...
	void setConnected(boolean connect){
		servosSetConnected(driver,connect);
	}
	void connect(void){
		servosConnect(driver);
	}
	void disconnect(void){
		servosDisconnect(driver);
	}
	void setSpeed(DRIVE_SPEED speed){
		servosSetSpeed(driver, speed);
	}

	void center(void){
		_servosCenter(driver->servos,driver->num_servos);
	}
private:
	SERVO_DRIVER* driver;
};
