...
typedef struct s_stepper {
	...
	const struct c_stepper_impl* sclass;// The driver class that controls this stepper motor
...
typedef struct s_stepper_driver {
	const STEPPER_LIST* const motors;		// The motors
...
void __stepperTick(STEPPER_DRIVER* driver);
#endif
...
public:
	StepperMotor(STEPPER_MOTOR* c) : Actuator(&c->actuator) {
		m_stepper = c;
	}
	// Move the stepper through a given number of steps
	void step(int16_t steps) const{
		__stepper_move(m_stepper, steps);
	}

	DRIVE_SPEED getActualSpeed(void) const{
		return __stepper_getActualSpeed(m_stepper);
	}

	// Between 0 and steps per revolution -1
	uint16_t getActualPosition(void) const{
		return __stepper_getPosition(m_stepper);
	}

	int16_t getStepsRemaining(void) const{
		return __stepper_getStepsRemaining(m_stepper);
	}

	// the number of steps for 360 degrees
	uint16_t getStepsPerRevolution(void) const{
		return m_stepper->stepsPerRevolution;
	}


private:
	STEPPER_MOTOR* m_stepper;
};
#endif
