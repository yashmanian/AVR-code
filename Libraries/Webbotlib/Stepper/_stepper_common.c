...
static void impl_act_setConnected(__ACTUATOR* act,boolean connected){
	...
	const STEPPER_MOTOR_IMPL* driver=motor->sclass;
...
const __ACTUATOR_DRIVER_CLASS PROGMEM c_stepper_actuator = MAKE_ACTUATOR_DRIVER_CLASS(&impl_act_setSpeed,&impl_act_setConnected);
	...
	for(int i= driver->num_motors-1;i>=0;i--){
		...
		if(motor->actualSpeed && motor->actuator.connected){
			...
			if(tick){
				...
				const struct c_stepper_impl* clazz = motor->sclass;
...
void stepper_init(STEPPER_DRIVER* driver, const Timer* timer){
	...
	if(!valid){
		...
		// Set the overflow
		timerOverflowAttach(timer, &stepperTick, driver);
