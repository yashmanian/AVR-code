...
typedef struct struct_cBuffer{
	...
	Reader		reader;				// a reader for the buffer
	Writer		writer;				// a writer for the buffer
} cBuffer;

// Constructor
#define MAKE_BUFFER(bytes) {bytes,sizeof(bytes),0,0,null,null}
#define MAKE_BUFFER_STREAM(bytes,reader,writer) {bytes,sizeof(bytes),0,0,reader,writer}
...
static __inline__ Reader bufferGetReader(const cBuffer* buffer){
	return buffer->reader;
}
static __inline__ Writer bufferGetWriter(const cBuffer* buffer){
...
public:
	...
	virtual int write(uint8_t data){
		return (bufferPut(m_cstr,data)) ? data : EOF;
	}
	virtual int read(void){
		uint8_t rtn;
		return ( bufferGet(m_cstr,&rtn) ) ? ((int)rtn)&0xff : EOF;
	}

	void flush(void)const{
		return bufferFlush(m_cstr);
	}

	boolean isFull(void)const{
		return bufferIsFull(m_cstr);
	}

	size_t freeSpace(void)const{
		return bufferFreeSpace(m_cstr);
	}

	size_t bytesUsed(void)const{
		return bufferBytesUsed(m_cstr);
	}
