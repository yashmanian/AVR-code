...
#define MAKE_SW_UART_BUFFERED(rxBuf,txBuf,timer,rxpin,txpin,inv,reader,writer) \
	{ MAKE_UART_COMMON(rxBuf,txBuf,&c_sw_uart##timer,rxpin,txpin,reader,writer) , \
	0,0,0,0,0,0,inv }
*/
// Make the above macros deprecated - you should use the ones below instead
#pragma GCC poison MAKE_SW_UART_BUFFERED

// Constructor for a software UART with buffer on Tx and Rx
#define MAKE_SW_UART_Rx_Tx(rxBuf,txBuf,timer,rxpin,txpin,inv,reader,writer) \
	{ MAKE_UART_COMMON_Rx_Tx(rxBuf,txBuf,&c_sw_uart##timer,rxpin,txpin,reader,writer) , \
	0,0,0,0,0,0,inv }

// Constructor for a software UART with buffer on Rx only
#define MAKE_SW_UART_Rx(rxBuf,timer,rxpin,txpin,inv,reader,writer) \
	{ MAKE_UART_COMMON_Rx(rxBuf,&c_sw_uart##timer,rxpin,txpin,reader,writer) , \
	0,0,0,0,0,0,inv }

// Constructor for a software UART with buffer on Tx only
#define MAKE_SW_UART_Tx(txBuf,timer,rxpin,txpin,inv,reader,writer) \
	{ MAKE_UART_COMMON_Tx(txBuf,&c_sw_uart##timer,rxpin,txpin,reader,writer) , \
	0,0,0,0,0,0,inv }

// Constructor for a software UART with no buffering at all
#define MAKE_SW_UART(timer,rxpin,txpin,inv,reader,writer) \
	{ MAKE_UART_COMMON(&c_sw_uart##timer,rxpin,txpin,reader,writer) , \
	0,0,0,0,0,0,inv }
...
public:
	UartSW(SW_UART* uart) : Uart(&uart->_uart_){};
private:
};
#endif
