...
extern const IOPin* PROGMEM const PCINT_PINS[];
...
void pin_change_attach(const IOPin* io,PinChangeCallback callback, void* data){
	...
	if(inx==-1){
		...
		if(inx<=7){
#ifdef PCMSK0
			PCMSK0 |= BV(inx);		// enable interrupts for this pin
			PCICR  |= BV(PCIE0);	// enable interrupts for PCINT0..7
#endif
#ifdef PCMSK1
		}else if(inx<=15){
			PCMSK1 |= BV((inx & 7));// enable interrupts for this pin
			PCICR  |= BV(PCIE1);	// enable interrupts for PCINT8..15
#endif
#ifdef PCMSK2
		}else if(inx<=23){
			PCMSK2 |= BV((inx & 7));// enable interrupts for this pin
			PCICR  |= BV(PCIE2);	// enable interrupts for PCINT16...23
#endif
#ifdef PCMSK3
		}else if(inx<=31){
			PCMSK3 |= BV((inx & 7));// enable interrupts for this pin
			PCICR  |= BV(PCIE3);	// enable interrupts for PCINT24..31
#endif
		}
	}
