...
typedef struct s_spi_abstract_device{
	...
	uint8_t			fillerByte;	// The byte to send when just receiving - often 0xFF for MODE0 or 2, or 0x00 for MODE1 or 3
} SPI_ABSTRACT_DEVICE;
...
typedef struct s_spi_abstract_bus {
	const struct /*PROGMEM*/ c_spi*	const sclass;	// The type of implementation
	const SPI_DEVICE_LIST* const devices;		// The list of attached devices
...
class SpiBus{
public:
	SpiBus(SPI_ABSTRACT_BUS* cstr){
		m_cstr = cstr;
	}

	void setClockPrescaler(SPI_CLOCK prescale) const{
		spiAbstractBusSetClock(m_cstr,prescale);
	}

	SPI_CLOCK getClockPrescaler(void) const{
		return spiAbstractBusGetClock(m_cstr);
	}

private:
	SPI_ABSTRACT_BUS* m_cstr;
};


class SpiDevice {
public:
	SpiDevice(SPI_ABSTRACT_DEVICE* cstr){
		m_cstr = cstr;
	}

	// You can select it
	void select(boolean active)const{
		spiAbstractDeviceSelect(m_cstr,active);
	}

	// Write a byte with no response
	void write(uint8_t byte) const{
		spiAbstractDeviceSendByte(m_cstr,byte);
	}
	// Write a number of bytes from a buffer
	void write(const void* src, size_t size) const{
		spiAbstractDeviceSendBytes(m_cstr, src, size);
	}


	// Send a byte and receive a response
	uint8_t xfer(uint8_t byte) const{
		return spiAbstractDeviceXferByte(m_cstr, byte);
	}

	// Get a byte from the device
	uint8_t get(void)const{
		return spiAbstractDeviceReceiveByte(m_cstr);
	}


	// Receive a number of bytes into a buffer
	void read(void* dst, size_t size) const{
		spiAbstractDeviceReceiveBytes(m_cstr, dst, size);
	}


private:
	SPI_ABSTRACT_DEVICE* m_cstr;
};

class SpiGenericDevice : public SpiDevice {
public:
	SpiGenericDevice(SPI_GENERIC_DEVICE* cstr) : SpiDevice(&cstr->_device_){
	}
};
#endif
