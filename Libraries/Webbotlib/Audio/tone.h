...
typedef struct /*PROGMEM*/ s_tone_class {
	void (*pgm_play)(uint16_t frequency, uint32_t durationMS );
	void (*pgm_stop)(void);
} TONE_CLASS;
#define MAKE_TONE_CLASS(play,stop) {play,stop}

// Forward def
struct s_tone_player;

// Define a callback for when a tone is finished
typedef void (*ToneCallback)(struct s_tone_player *player);


typedef struct s_tone_player {
	const TONE_CLASS* clazz;	// The player class
	...
	void*  data;				// Pointer back to the C++ class
...
#define MAKE_TONE_PLAYER( timer, pin ) {null ,timer,pin, 0, FALSE, NULL, FALSE,FALSE,NULL}
...
void tonePlayRTTTL(TONE_PLAYER* player, const char str[]);
...
class TonePlayer {
public:
	TonePlayer(TONE_PLAYER* cstr){
		m_cstr = cstr;
		m_callback = null;
		cstr->data = this;
	}

	void play(uint16_t frequency, uint32_t durationMS) const{
		tonePlay(m_cstr, frequency, durationMS );
	}
	...
	// Are we currently playing a tone
	boolean isPlaying(void)const{
		return m_cstr->playing;
	}

	// Support Ring Tone Text Transfer Language RTTTL
	// Play a tune from program memory
	void play(const char tune[]){
		tonePlayRTTTL(m_cstr, tune);
	}


	// Attach a callback for when the current note has stopped playing
	void attach( void (* callback)(TonePlayer& player)){
		m_callback = callback;
		toneAttach(m_cstr, &TonePlayer::callback);
	}
...
protected:
	// Generic call back
	static void callback(TONE_PLAYER *player){
		TonePlayer* inst = (TonePlayer*)(player->data);
		if(inst->m_callback){
			inst->m_callback(*inst);
		}
	}
...
private:
	TONE_PLAYER* m_cstr;
	void (* m_callback)(TonePlayer& player);
};
#endif
