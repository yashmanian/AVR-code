...
extern uint8_t _speech_timeFactor;
#endif
...
static const SOUND_INDEX PROGMEM SoundIndex[] = {
	...
	extern void _speech_set_volume(uint8_t b);
//	static const TimerCompare* channel;
//	extern const IOPin* _speech_pwmPin;
#else
...
static void pause(uint8_t delay){
	#if !defined(_WINDOWS_)
	uint8_t r;
	for(r=_speech_timeFactor; r>0; r--){
		_delay_loop_1(delay);
	}
	#endif
...
static void soundOff(void){
	#if !defined(_WINDOWS_)
//	pwmSetDutyCycle(_speech_pwmPin,50);	// Set 50% duty cycle
	_speech_set_volume(8);
	#endif
}
