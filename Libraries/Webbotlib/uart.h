...
*/
// Make the above macros deprecated - you should use the ones below instead
#pragma GCC poison MAKE_UART_BUFFERED
#pragma GCC poison MAKE_UART

// Construct a hardware UART with buffering on both Tx and Rx
#define MAKE_HW_UART_Rx_Tx(rxBuf,txBuf,statusA,statusB,baudL,baudH,data,spi,u2x,rx,tx,reader,writer) \
	{ MAKE_UART_COMMON_Rx_Tx(rxBuf,txBuf,&c_hw_uart,rx,tx,reader,writer) , \
	_SFR_MEM_ADDR(statusA),_SFR_MEM_ADDR(statusB),_SFR_MEM_ADDR(baudL),_SFR_MEM_ADDR(baudH),_SFR_MEM_ADDR(data),spi,u2x}

// Construct a hardware UART with buffering on Rx only
#define MAKE_HW_UART_Rx(rxBuf,statusA,statusB,baudL,baudH,data,spi,u2x,rx,tx,reader,writer) \
	{ MAKE_UART_COMMON_Rx(rxBuf,&c_hw_uart,rx,tx,reader,writer) , \
	_SFR_MEM_ADDR(statusA),_SFR_MEM_ADDR(statusB),_SFR_MEM_ADDR(baudL),_SFR_MEM_ADDR(baudH),_SFR_MEM_ADDR(data),spi,u2x}

// Construct a hardware UART with buffering on Tx only
#define MAKE_HW_UART_Tx(txBuf,statusA,statusB,baudL,baudH,data,spi,u2x,rx,tx,reader,writer) \
	{ MAKE_UART_COMMON_Tx(txBuf,&c_hw_uart,rx,tx,reader,writer) , \
	_SFR_MEM_ADDR(statusA),_SFR_MEM_ADDR(statusB),_SFR_MEM_ADDR(baudL),_SFR_MEM_ADDR(baudH),_SFR_MEM_ADDR(data),spi,u2x}

// Construct a hardware UART with no buffering
#define MAKE_HW_UART(statusA,statusB,baudL,baudH,data,spi,u2x,rx,tx,reader,writer) \
	{ MAKE_UART_COMMON(&c_hw_uart,rx,tx,reader,writer) , \
	_SFR_MEM_ADDR(statusA),_SFR_MEM_ADDR(statusB),_SFR_MEM_ADDR(baudL),_SFR_MEM_ADDR(baudH),_SFR_MEM_ADDR(data),spi,u2x}
...
/*
 * Now deprecated you need to use the getWriter function on the 'main device' you are talking to
 * If its just a uart then see uartGetWriter, uartGetReader
 *
*/
