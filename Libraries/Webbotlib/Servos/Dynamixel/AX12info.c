...
//		uartFlushReceiveBuffer(uart);
		...
		CRITICAL_SECTION{						// At 1Mb baud we have to poll for chars or they get missed
				...
				if(reply[3]==sizeof(reply)-4){
					...
					if(chksum==0xff){
						...
						if(rtn == 0){
							...
							// Get the current speed -0x3ff to +0x3ff = +- 114 RPM
							// 0x3ff => 1023
							int16_t speed = reply[7] | (reply[8]<<8);
							if(speed & 0x400){
								speed = - (speed & 0x3ff);
							}
							// Save the current rotation speed as RPM
							servo->info.rpm=interpolate(speed, -1023, 1023, -114, 114);


							if(servo->continuous){
								// Motor mode

								// Save the current rotation speed as a DRIVE_SPEED
								servo->info.speed=interpolate(speed, -1023, 1023, DRIVE_SPEED_MIN, DRIVE_SPEED_MAX);
								...
								servo->info.speed=interpolate(pos, 0, 1023, DRIVE_SPEED_MIN, DRIVE_SPEED_MAX);
