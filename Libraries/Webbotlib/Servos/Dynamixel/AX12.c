...
void ax12Init(DYNAMIXEL_AX12_DRIVER* driver, BAUD_RATE baud){
	...
	if(driver->directionPin){
		...
		if(driver->ax12uart->_uart_.classdef == &c_hw_uart){
	...
	for(int i= driver->num_servos-1;i>=0;i--){
		...
		if(servo->continuous){
			...
			// Try to move as fast as possible between locations
			ax12SetMOVING_SPEED(servo,0);
...
static const __ACTUATOR_DRIVER_CLASS PROGMEM c_ax12 = MAKE_ACTUATOR_DRIVER_CLASS(&setSpeed,null);
