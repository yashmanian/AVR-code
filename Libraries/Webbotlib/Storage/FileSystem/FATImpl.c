	...
	if(buf){
		...
		for(int p=3; p>=0; p--){
			...
			if(type == PT_FAT12  || type == PT_FAT16A || type == PT_FAT16  ||  type == PT_FAT32  || type == PT_FAT32A || type == PT_FAT16B){
				...
				disc->partitionStartSector = partition->LBA_begin;	// 8192
				disc->partitionNumSectors = partition->numSectors;	// 7951360 = 3.8Gb
				rtn = TRUE;
			}
		}
		if(!rtn){
...
static boolean _ioReadSector(const DISK *disc,SECTOR absSector,void* dta){
	...
	boolean rtn=  read(disc->device,absSector,dta);

#ifdef FAT_DEBUG
	memoryDump(stdout,dta, 0, 512);
#endif
...
#ifdef FAT_DEBUG
	{
		PRINTF(stdout,"_fatGetNextClusterChain: Rel=%lu, Actual=%lu, Contig=%lu\r\n",chain->relativeCluster,chain->currentCluster,chain->contiguousCount);
	}
#endif
...
#ifdef FAT_DEBUG
		PRINTF(stdout,"_fatNavigateTo Nav From start cluster: %lu\r\n",nav->currentCluster);
#endif
	}
	...
	while(nav->relativeCluster!=relativeCluster){
...
#ifdef FAT_DEBUG
		PRINTF(stdout,"_fatNavigateTo: Current:%lu, Target:%lu, ActualCluster:%lu\r\n",nav->relativeCluster,relativeCluster,nav->currentCluster);
#endif
		...
		if(nav->contiguousCount!=0){
			...
			if( !_fatGetNextClusterChain(disk,nav)){
				// We've hit the end
#ifdef FAT_DEBUG
				PRINTF(stdout,"_fatNavigateTo : Reached end\r\n");
#endif
				return(FALSE);
			}
		}
	}
	// We have navigated to the correct position
#ifdef FAT_DEBUG
	{
		PRINTF(stdout,"_fatNavigateTo found: Current:%lu, ActualCluster:%lu\r\n",nav->relativeCluster,nav->currentCluster);
	}
#endif
	return(TRUE);
}
...
#ifdef FAT_DEBUG
	{
		PRINTF(stdout,"Start Get Free Cluster\r\n");
	}
#endif
...
#ifdef FAT_DEBUG
		PRINTF(stdout,"Cluster %lu, next=%lu\r\n",current,next);
#endif
...
#ifdef FAT_DEBUG
	PRINTF(stdout,"End Get Free Cluster=%lu\r\n",rtn);
#endif
...
#ifdef FAT_DEBUG
	PRINTF(stdout,"End extend: First Cl=%lu, End Cl=%lu, Current=%lu\r\n",nav->startCluster,lastCluster,nav->currentCluster);
#endif
...
static boolean _fatInit(DISK* disk){
	...
	disk->volume.bytesPerSector=get_uint16(buf,0x0B);		//  512
	disk->volume.sectorsPerCluster=*((char*)(buf+0x0D));	//   64
	disk->volume.reservedSectorCount=get_uint16(buf,0x0E);	// 6252
	disk->volume.numberOfFats=*((char*)(buf+0x10));			//    2
	disk->volume.rootDirEntryCount=get_uint16(buf,0x11);	//    0
	disk->volume.rootDirCluster=get_uint32(buf,0x2C);		//	  2
	uint16_t FatSectorCount16=get_uint16(buf,0x16);			// 	  0
	uint32_t FatSectorCount32=get_uint32(buf,0x24);			//  970
	uint16_t SectorCount16=get_uint16(buf,0x13);			//    0
	uint32_t SectorCount32=get_uint32(buf,0x20);			// 7,051,360
	...
	disk->firstSectorRootDir = disk->volume.reservedSectorCount +
	                         (disk->volume.numberOfFats * disk->fatSectorCount); //=6,252 + 2*970=8192
	if(disk->fatType==32){
	...
	disk->firstClusterCurrentDir = _diskGetFirstClusterRootDir(disk); // = 2

#ifdef FAT_DEBUG
	PRINTF(stdout,"FAT StartSec:%u NumSec:%lu\r\n",disk->volume.reservedSectorCount,disk->fatSectorCount);
	PRINTF(stdout,"RootDir Sec:%lu\r\n",disk->firstSectorRootDir);
	PRINTF(stdout,"Sec/Cluster:%u\r\n",disk->volume.sectorsPerCluster);
#endif
...
#ifdef FAT_DEBUG
	PRINTF(stdout,"Start: Find File '%s'\r\n",filename);
#endif
...
void _dirCreateDirectoryEntry(const DISK* disk,const DIR_ENTRY *filerec,const DIR_POSITION *loc){
	#ifdef FAT_DEBUG
	PRINTF(stdout,"Start _dirCreateDirectoryEntry\r\n");
	#endif
	...
	#ifdef FAT_DEBUG
	PRINTF(stdout,"End _dirCreateDirectoryEntry\r\n");
	#endif
...
#ifdef FAT_DEBUG
	PRINTF(stdout,"Start _dirFindFreeFile '%s'\r\n",filename);
#endif


	if(_fileFindFile(disk,filename,loc,&targetdir)){
#ifdef FAT_DEBUG
		PRINTF(stdout,"End _dirFindFreeFile '%s': already exists\r\n",filename);
#endif
...
#ifdef FAT_DEBUG
		PRINTF(stdout,"End _dirFindFreeFile '%s': invalid name\r\n",filename);
#endif
		return(FALSE);
	}
...
#ifdef FAT_DEBUG
		PRINTF(stdout,"End _dirFindFreeFile '%s': found free slot\r\n",filename);
#endif
		return(TRUE);
	}else{
...
#ifdef FAT_DEBUG
			PRINTF(stdout,"End _dirFindFreeFile '%s': directory full\r\n",filename);
#endif
			return(FALSE);
		}else{
			if(_dirFindinDir(disk,ffname,targetdir,loc,DIRFIND_FREE)){
...
#ifdef FAT_DEBUG
	PRINTF(stdout,"End _dirFindFreeFile '%s': failed\r\n",filename);
#endif
