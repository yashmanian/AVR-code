...
class FileIterator : private FILE_ITERATOR {
public:
	boolean findNext(void){
		return fileFindNext(this);
	}
	...
	boolean isDirectory(void) const{
		return (attribute.flags.isDirectory) ? TRUE : FALSE;
	}
	operator FILE_ITERATOR*(){
		return this;
	}
};

class File : private FATFILE, public Stream {
public:
	...
	// When it goes out of scope then close the file
	~File(){
		close();
	}
	...
	virtual int write(uint8_t data){
		size_t len = fileWriteNext(this, 1,&data);
		return (len==1) ? data : EOF;
	}
	virtual int read(void){
		uint8_t data;
		size_t len = fileReadNext(this,1,&data);
		return (len==1) ? data : EOF;
	}
	...
	boolean setPos(uint32_t pos){
		return fileSetPos(this,pos);
	}

	uint32_t getPos(void) const{
		return fileGetPos(this);
	}

	uint32_t size(void) const{
		return fileGetSize(this);
	}

	size_t read(uint32_t pos, size_t len, void* dst ){
		return fileRead(this,pos,len,dst);
	}
	size_t read(size_t len, void* dst ){
		return fileReadNext(this,len,dst);
	}

	size_t write(uint32_t pos, size_t len, const void* src ){
		return fileWrite(this,pos,len,src);
	}
	size_t write(size_t len, const void* src ){
		return fileWriteNext(this,len,src);
	}
...
public:
	...
	boolean isReady(void) const{
		return m_cstr->initialised;
	}

	boolean flush(void)const{
		return diskFlush(m_cstr);
	}

	uint32_t freeKB(void)const{
		return diskFreeSpace(m_cstr);
	}

	int8_t mkDir(const char* dirname)const{
		return diskMkdir(m_cstr,dirname);
	}

	int8_t open( File& file, const char* filename, char mode)const{
		return fileOpen(m_cstr, file,filename,mode);
	}

	boolean remove(const char* filename)const{
		return fileDelete(m_cstr,filename);
	}

	boolean exists(const char* filename)const{
		return fileExists(m_cstr,filename);
	}

	boolean findFirst(FileIterator& list, const char* dirname) const{
		return fileFindFirst(m_cstr,list,dirname);
	}
