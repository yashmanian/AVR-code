...
#ifdef FAT_DEBUG
	PRINTF(stdout,"_fileInitFile FileSize:%lu StartCluster:%lu \r\n",file->fileSize,file->startCluster);
#endif
...
static void _allocateFirstCluster(DISK* disk, FATFILE* file, DIR_ENTRY* dir, const DIR_POSITION *loc){
	#ifdef FAT_DEBUG
	PRINTF(stdout,"Start _allocateFirstCluster\r\n");
	#endif
	...
	#ifdef FAT_DEBUG
	PRINTF(stdout,"End _allocateFirstCluster\r\n");
	#endif
}
...
#ifdef FAT_DEBUG
					PRINTF(stdout,"Create dir '%11.11s' DirSector:%lu Entry:%u\r\n",fatfilename,loc.sector,loc.entryInSector);
#endif
...
void fileFlush(FATFILE* file){
	if(_fileGetAttr(file,FILE_STATUS_WRITE)){
		...
		PRINTF(stdout,"Flush File: First Cl=%lu, NumCl=%lu\r\n",file->startCluster,cnt);
#endif
...
#ifdef FAT_DEBUG
//PRINTF(stdout,"_fileRequiredCluster Current Num Clusters=%lu\r\n",hc);
#endif
