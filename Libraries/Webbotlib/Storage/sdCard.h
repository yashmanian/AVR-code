...
public:
	...
	boolean readSectors(uint32_t absSector,void* dta,uint8_t numSectors){
		return sdCardRead(m_cstr,absSector,dta,numSectors);
	}
	boolean writeSectors(uint32_t absSector,const void* dta,uint8_t numSectors){
		return sdCardWrite(m_cstr,absSector,dta,numSectors);
	}
	uint32_t totalSectors(void) const{
		return m_cstr->numSectors;
	}
private:
	SD_CARD* m_cstr;
};
