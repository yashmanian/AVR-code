...
#define MAKE_I2C_EEPROM_AT24C1024B(a2,a1) { \
	MAKE_I2C_DEVICE( 0xA0 | ((a2 & 1)<<3) | ((a1 & 1)<<2) ), \
	2, 256, 128*KB}
...
class I2C_Eeprom {
public:
	I2C_Eeprom(I2C_EEPROM* eeprom cstr){
		m_cstr = cst;
	}

	uint8_t readByte(EEPROM_ADDR addr) const{
		return i2cEEPROM_readByte(m_cstr, addr);
	}

	// Read a sequence of byte
	void readBytes(EEPROM_ADDR addr, void* dest, size_t numBytes) const{
		i2cEEPROM_readBytes(m_cstr, addr, dest, numBytes);
	}

	// Write a byte to the eeprom
	void writeByte(EEPROM_ADDR addr, uint8_t data) const{
		i2cEEPROM_writeByte(m_cstr,addr,data);
	}

	// Write a sequence of bytes
	void writeBytes(EEPROM_ADDR addr, const void* src, size_t numBytes) const{
		i2cEEPROM_writeBytes(m_cstr, addr, src, numBytes);
	}

	EEPROM_ADDR totalBytes(void) const{
		return i2cEEPROM_totalBytes(m_cstr);
	}
