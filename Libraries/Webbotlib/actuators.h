...
public:
	...
	void setConnected(boolean con){
		__act_setConnected(m_cstr,con);
	}
	void connect(void){
		__act_setConnected(m_cstr,TRUE);
	}
	void disconnect(void){
		__act_setConnected(m_cstr,FALSE);
	}
	boolean isConnected(void) const{
		return __act_isConnected(m_cstr);
	}
	boolean isInverted(void) const{
		return __act_isInverted(m_cstr);
	}
private:
	__ACTUATOR* m_cstr;
};
