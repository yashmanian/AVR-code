...
typedef struct s_sensor{
	const SENSOR_CLASS* sclass;					// The class it belongs to
	TICK_COUNT  lastRead;						// The time when it was last read
	boolean  	initialised;					// Has it been initialised
} SENSOR;

#define MAKE_SENSOR(sclass) {sclass,(TICK_COUNT)0, FALSE}
...
class Sensor{
public:
	//	virtual void dumpTo(Stream& str) const = 0;
	virtual void dumpTo(FILE* str) const = 0;
	Sensor(SENSOR* cstr){m_cstr = cstr;};
	boolean read(void) {
		return _sensorRead_(m_cstr);
	}
	void dump(void) const{
		dumpTo(stdout);
	}
	TICK_COUNT getTimeLastRead(void)const{
		return m_cstr->lastRead;
	}
private:
	SENSOR* m_cstr;
};
