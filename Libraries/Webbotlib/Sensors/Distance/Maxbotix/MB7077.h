...
class Mb7077 : public Distance{
public:
	Mb7077(Maxbotix_MB7077* c) : Distance(&c->distance){
		m_cstr = c;
	}
	void setInWater(boolean water) const{
		m_cstr->inWater = water;
	}
	boolean isInWater(void) const{
		return m_cstr->inWater;
	}
private:
	Maxbotix_MB7077* m_cstr;
};
#endif
