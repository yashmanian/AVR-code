...
// The maximum value of a DISTANCE_TYPE
...
void _distanceDumpTo(FILE* f,const DISTANCE* device);
#define distanceDumpTo(f,device) _distanceDumpTo(f,&device.distance);
#define distanceDump(device) _distanceDumpTo(stdout,&device.distance);

#ifdef __cplusplus
}
class Distance : public Sensor{
public:
	Distance(DISTANCE* cstr) : Sensor(&cstr->sensor){
		m_cstr = cstr;
	}
