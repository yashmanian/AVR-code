...
static void getReading(Devantech_SRF02* device){
	if(device){
		uint8_t response[4];
		const I2C_DEVICE* i2c = &(device->i2cInfo);
		...
		INTERRUPTABLE_SECTION{
			boolean ok;
			do{
				ok = i2cMasterReadRegisters(i2c, 0, sizeof(response), response);
			}while(ok==false || response[0]==255 );
		}
...
const DISTANCE_CLASS PROGMEM c_Devantech_SRF02 = MAKE_DISTANCE_CLASS(null,&__srf02_read,0,70);
