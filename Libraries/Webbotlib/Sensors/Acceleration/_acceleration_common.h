...
#define accelerometerDumpTo(f,device) _accelerometerDumpTo(f,&device.accelerometer);
#define accelerometerDump(device) _accelerometerDumpTo(stdout,&device.accelerometer);
...
public:
	...
	Accelerometer(ACCELEROMETER* accel) : Sensor(&accel->sensor){
		m_accel=accel;
	}

//	virtual void dumpTo(Stream& str) const;
	virtual void dumpTo(FILE* str = stdout) const{
		_accelerometerDumpTo(str,m_accel);
	}

	void calibrateX(ACCEL_TYPE minx, ACCEL_TYPE maxx){
		__accelerometerCalibrateX(m_accel,minx,maxx);
	}
	void calibrateY(ACCEL_TYPE miny, ACCEL_TYPE maxy){
		__accelerometerCalibrateY(m_accel,miny,maxy);
	}
	void calibrateZ(ACCEL_TYPE minz, ACCEL_TYPE maxz){
		__accelerometerCalibrateZ(m_accel,minz,maxz);
	}

	void calibrate(ACCEL_TYPE minx, ACCEL_TYPE maxx,ACCEL_TYPE miny, ACCEL_TYPE maxy,ACCEL_TYPE minz, ACCEL_TYPE maxz){
		__accelerometerCalibrateX(m_accel,minx,maxx);
		__accelerometerCalibrateY(m_accel,miny,maxy);
		__accelerometerCalibrateZ(m_accel,minz,maxz);
	}

	ACCEL_TYPE getX(void) const {
		return m_accel->x_axis_mG;
	}

	ACCEL_TYPE getY(void) const {
		return m_accel->y_axis_mG;
	}

	ACCEL_TYPE getZ(void) const {
		return m_accel->z_axis_mG;
	}
