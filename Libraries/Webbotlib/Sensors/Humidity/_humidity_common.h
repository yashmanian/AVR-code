...
void _humidityDumpTo(FILE* f,const HUMIDITY* device);
#define humidityDumpTo(f,device) _humidityDumpTo(f,&device.humidity);
#define humidityDump(device) _humidityDumpTo(stdout,&device.humidity);
...
public:
	Humidity(HUMIDITY* cstr) : Sensor(&cstr->sensor){
		m_cstr = cstr;
	}
//	virtual void dumpTo(Stream& str) const;
	virtual void dumpTo(FILE* str=stdout) const{
		_humidityDumpTo(str,m_cstr);
	}

	HUMIDITY_TYPE getHumidity(void) const{
		return m_cstr->percent;
	}
