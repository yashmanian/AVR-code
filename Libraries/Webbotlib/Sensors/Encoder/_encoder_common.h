...
#define MAKE_ENCODER_SENSOR(class, inverted, numStripes,interpolate) {MAKE_SENSOR(&(class.sensor)), \
		...
		(TICK_COUNT)0, \
		(ENCODER_TYPE)0, \
		inverted, numStripes, { {0,0}, {0,0} }, 0, interpolate}
...
void _encoderDumpTo(FILE* f,const ENCODER* device);
#define encoderDumpTo(f,device) _encoderDumpTo(f,&device.encoder);
#define encoderDump(device) _encoderDumpTo(stdout,&device.encoder);
...
public:
	Encoder(ENCODER* cstr) : Sensor(&cstr->sensor){
		m_cstr = cstr;
	}

	ENCODER_TYPE getTicks(void) const{
		return m_cstr->value;
	}

	void subtract(ENCODER_TYPE count){
		_encoderSubtract(m_cstr,count);
	}

	uint16_t ticksPerRevolution(void) const{
		return _encoderTicksPerRevolution(m_cstr);
	}

	void setInterpolating(boolean state){
		if(state!=m_cstr->interpolate){
			m_cstr->interpolate = FALSE; // turn off whilst changing
			m_cstr->durationOfLastTick = 0;
			m_cstr->timeSinceLastTick=0;
			m_cstr->interpolate = state;
		}
	}

	boolean isInterpolating(void) const{
		return m_cstr->interpolate;
	}

	TICK_COUNT getTimeSinceLastTick(void) const{
		return m_cstr->timeSinceLastTick;
	}

	TICK_COUNT getDurationOfLastTick(void) const{
		return m_cstr->durationOfLastTick;
	}

//	virtual void dumpTo(Stream& str) const;
	virtual void dumpTo(FILE* str=stdout) const{
		_encoderDumpTo(str,m_cstr);
	}
