...
static size_t getResponse(AVRCAM* camera, char chStart, char chEnd, size_t minLen, size_t maxLen) {
	...
	while(!clockHasElapsed(startTime, TIMEOUT)) {
...
const CAMERA_CLASS PROGMEM c_avrcam = MAKE_CAMERA_CLASS( &init, &xres, &yres, AVRCAM_BINS, &getBlobs, &getPixel, &getVersion, &_cameraSetBin_RGB);
