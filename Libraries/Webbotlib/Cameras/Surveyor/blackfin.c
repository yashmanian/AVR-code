...
int __blackfin_get_args(const BLACKFIN_CAMERA* camera, int32_t * values, size_t numValues, boolean trace){
	...
	while(numValues>0 && received!='\n'){
		...
		while(1){
			received = __uartGetByte(camera->_camera_.uart);
			if(received!=-1){
				c = received;
...
retry:
	...
	__uartFlushReceiveBuffer(camera->_camera_.uart);

	// Send the command to the camera
	char* str = _blackfin_command;
	while (*str)
		_uartSendByte(camera->_camera_.uart, *str++);
	goto listen;
...
static void _blackfinInit(CAMERA* cam){
...
//	_uartInit(camera->camera, 115200);
	...
	if(camera->_camera_.uart->rxBuffer== null){
...
static uint16_t  xres(CAMERA* cam){
	BLACKFIN_CAMERA* camera = (BLACKFIN_CAMERA*)cam;
	uint16_t rtn = 0;
	switch(camera->res){
	case BLACKFIN_160_BY_120:
		rtn = 160;
		break;
	case BLACKFIN_320_BY_240:
		rtn = 320;
...
static uint16_t  yres(CAMERA* cam){
	BLACKFIN_CAMERA* camera = (BLACKFIN_CAMERA*)cam;
	uint16_t rtn = 0;
	switch(camera->res){
	case BLACKFIN_160_BY_120:
...
// Create the class for the blackfin
const CAMERA_CLASS PROGMEM c_blackfin = MAKE_CAMERA_CLASS(
		&_blackfinInit,
		&xres,
		&yres,
		10 /* BINS */,
		&_blackfinDetectBlobs,
		&_blackfinGetPixel,
		&getVersion,
		&_cameraSetBin_YUV);
