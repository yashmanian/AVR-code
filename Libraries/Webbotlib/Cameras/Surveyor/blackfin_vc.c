...
static void blackfinSetBinRange(BLACKFIN_CAMERA* camera,uint8_t bin, const COLOR* min, const COLOR* max){
...
boolean blackfinSendBins(BLACKFIN_CAMERA* camera){
	...
	uint8_t maxBins = _cameraNumColorBins(&camera->_camera_);
	for (uint8_t index=0; index<maxBins; index++) {
		CAMERA_BIN* bin = &camera->_camera_.bins[index];
		if(bin->dirty){
			blackfinSetBinRange(camera,index, &bin->min, &bin->max);
			bin->dirty = FALSE;
		}
	}
	return TRUE;
